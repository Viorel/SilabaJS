...
const Silaba = ( function ()
{
    ...
    Impl.prototype.IncearcaPrefixe = function ()
    {
        ...
        let prefix_incert = this.CautaPrefix( Impl.Date.PI, l1 );
        let l2 = prefix_incert === undefined ? 0 : prefix_incert[0].length + 1;

        let prefix_cert = this.CautaPrefix( Impl.Date.PC, Math.max( l1, l2 ) );
        ...
        */
        ...
        if( prefix_cert !== undefined ) // (e si cel mai lung)
        {
            ...
            else
            {
                ...
                if( coada.length > 0 )
                {
                    let alt = new Impl( coada, { Secundar: true } );
    ...
    Impl.prototype.TrateazaPrefixIncert = function ( prefix )
    {
        ...
        if( this.lungime > lun_prefix )
        {
            ...
            {
                this.cratime = 1 << ( lun_prefix - 1 ); // (incert; silabele formate din constante vor fi corectate ulterior)
            }
        }
    ...
    Impl.prototype.IncearcaSecventeFonostatistice = function ()
    {
        ...
        {
            let element = Impl.Dictionar.FS[s];
            let cratime = element.c;
            let exceptii = element.e;
            ...
            for( let p = 0; ; ++p )
            {
                ...
                for( let i = 0; i < exceptii.length; ++i )
                {
                    let exceptie = exceptii[i];
                    let start = p - exceptie[0];
    ...
    Impl.prototype.CautaPrefix = function ( date, lungime_minima )
    {
        ...
        for( let l = date.LMax; l >= Math.max( date.LMin, lungime_minima ); --l )
        {
    ...
    let prepFS = function ()
    {
        ...
        if( Impl.Dictionar.CE !== undefined )
        {
            for( let fs in Impl.Dictionar.FS )
            {
                ...
                Impl.Dictionar.FS[fs] = { c: Impl.Dictionar.FS[fs], e: e };
            }
        }

        return Impl.Dictionar.FS;
    }
    ...
    const modul =
    {
        Desparte: function ( text )
        {
            ...
            for( ; ; )
            {
                ...
                if( a[1] !== undefined )
                {
                    let optiuni = { EvitaSilabeNeeconomice: true, EvitaSecventeNeelegante: true };
